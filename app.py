...
def _enclosing_ellipse_params(xy: np.ndarray):
    """
    Ellipse that encloses ALL points using covariance orientation scaled
    by max Mahalanobis distance.
    Returns (center, width, height, angle_deg) or None.
    """
    if xy.shape[0] < 3:
        return None
    ...
    k = float(np.sqrt(np.max(d2)))  # scale to enclose all points
...
def add_enclosing_ellipse(ax, x, y):
    xy = np.column_stack([x, y]).astype(float)
    params = _enclosing_ellipse_params(xy)
    if not params:
        return
    mu, w, h, ang = params
    e = Ellipse(
        xy=mu, width=w, height=h, angle=ang,
...
def plot_virtual_range(df: pd.DataFrame, clubs: list[str], title: str, draw_oval: bool):
    ...
    for c in clubs:
        ...
        if draw_oval:
            add_enclosing_ellipse(ax, sub["Dir_signed"].values, sub["Carry[yd]"].values)
...
draw_oval = st.sidebar.checkbox("Mostrar óvalo de dispersión", value=True)
